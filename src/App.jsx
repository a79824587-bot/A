...
const AUDIO_CONFIG = {
  ...
  seRows: [
    [
      { id: 'se1', name: '風魔法', url: 'https://www.image2url.com/r2/default/audio/1788624749112-b41a8fc0-e979-4f24-b37d-640e87ab8278.mp3' },
    ...
    [
      ...
      { id: 'se9', name: '神秘', url: 'https://www.image2url.com/r2/default/audio/1788624607965-09abde27-c46a-4c3e-811a-4176153b16e8.mp3' },
    ],
    [
      { id: 'se10', name: '氷魔法', url: 'https://www.image2url.com/r2/default/audio/1788597258450-46315a5b-3ba5-475a-a917-d02eb8bcc44b.mp3' },
      { id: 'se11', name: '踏み込む', url: 'https://www.image2url.com/r2/default/audio/1788597297745-57939090-7243-487f-9d31-285f90e3b350.mp3' },
      { id: 'se12', name: '雷', url: 'https://www.image2url.com/r2/default/audio/1788597345314-448ca94e-993e-4979-9890-9c0200249f5d.mp3' }
    ]
  ]
};
...
const fadeOutAndStop = (audio, targetVolume) => {
  ...
  const steps = 15; // 15ステップでより滑らかに
  const intervalTime = 10; // 計150msのフェードアウトでブツッというノイズを完全に防ぐ
  let currentStep = 0;

  const interval = setInterval(() => {
    currentStep++;
    const newVolume = Math.max(0, startVolume * (1 - currentStep / steps));
    if (audio && !isNaN(newVolume)) {
...
const useAudioPlayer = (initialUrl, isLoop = true) => {
  ...
  useEffect(() => {
    ...
    if (cleanUrl && !cleanUrl.startsWith('http://googleusercontent.com/')) {
      audio.src = cleanUrl;
    }
    ...
    if (isPlaying) {
      ...
      if (playPromise !== undefined) {
        playPromise.then(() => setIsPlaying(true)).catch(e => {
          console.error("BGM再生エラー(リトライします):", e);
          // 再生失敗時は即座にロードし直してリトライ（オープニング鳴りにくい問題の解消）
          audio.load();
...
const useSeTogglePlayer = (initialUrl) => {
  const [playState, setPlayState] = useState('IDLE'); // 'IDLE', 'SINGLE', 'LOOP'
  const cleanUrl = formatUrl(initialUrl);
  ...
  useEffect(() => {
    if (!cleanUrl || cleanUrl.startsWith('http://googleusercontent.com/')) return;
  ...
  const play = () => {
    ...
    const playPromise = audio.play();
    if (playPromise !== undefined) {
      playPromise.then(() => setPlayState('SINGLE')).catch(e => {
        console.error("SE再生エラー(リトライ):", e);
        audio.load();
        audio.play().then(() => setPlayState('SINGLE')).catch(() => setPlayState('IDLE'));
      });
    }
  };
...
const BgmTrack = ({ defaultName = "曲名〜", url }) => {
  ...
  const hasAudio = Boolean(cleanUrl && cleanUrl.trim() !== '' && !cleanUrl.startsWith('http://googleusercontent.com/'));
    ...
    <div className={`w-full max-w-[200px] rounded-2xl border p-3.5 transition-all duration-300 backdrop-blur-md relative overflow-hidden ${
      ...
      <div className="flex items-center justify-between gap-2.5">
        ...
        <button 
          onClick={togglePlay} 
          disabled={!hasAudio}
          className={`flex-1 h-9 rounded-xl font-black text-xs tracking-widest transition-all duration-200 active:scale-95 flex items-center justify-center gap-1.5 ${
            isPlaying 
      ...
      <div className="w-full h-11 sm:h-12 relative flex">
        ...
        {playState === 'IDLE' && (
          <button 
            ...
            disabled={!hasAudio}
            className={`w-full h-full rounded-xl border font-bold text-xs transition-all duration-75 flex items-center justify-center gap-1.5 relative overflow-hidden active:scale-90 ${
              hasAudio 
        ...
        {/* ② 1回再生中：止める / 連続再生 の分岐ボタン */}
        {playState === 'SINGLE' && (
          <div className="w-full h-full flex gap-0.5 animate-in fade-in zoom-in-95 duration-150">
            <button 
              onClick={stop}
              className="flex-1 h-full rounded-l-xl border border-red-400/80 bg-red-600 text-white font-bold text-[10px] sm:text-[11px] shadow-[0_0_15px_rgba(239,68,68,0.6)] active:bg-red-700 transition-colors"
            >
              止める
            </button>
            <button 
              onClick={setLoop}
              className="flex-[1.2] h-full rounded-r-xl border border-fuchsia-400/80 bg-fuchsia-600 text-white font-bold text-[10px] sm:text-[11px] shadow-[0_0_15px_rgba(217,70,239,0.6)] active:bg-fuchsia-700 transition-colors flex items-center justify-center"
            >
              連続再生
            </button>
          </div>
        ...
        {playState === 'LOOP' && (
          <button 
            onClick={stop}
            className="w-full h-full rounded-xl border border-fuchsia-400 bg-fuchsia-700 text-white font-bold text-xs shadow-[0_0_20px_rgba(217,70,239,0.8)] flex items-center justify-center gap-1.5 active:scale-90 transition-all animate-pulse"
          >
        ...
        <div className="flex flex-col gap-3.5 items-center">
          ...
          <div className="w-full flex flex-col relative mt-0.5">
            <div className="w-full flex items-center justify-between mb-1 px-1">
              ...
              <button
                onClick={stopAllSe}
                className="text-[10px] font-bold px-2.5 py-1 bg-red-950/80 hover:bg-red-900 border border-red-800/80 hover:border-red-500 text-red-300 rounded-lg transition-all active:scale-95 flex items-center gap-1 shadow-sm cursor-pointer"
              >
